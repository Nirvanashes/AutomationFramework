...
class InterfacePath(Base):
    ...
    protocol = Column(String, comment="协议:http，https协议")
    method = Column(String, comment="接口请求方式:get，post")
    headers = Column(String, comment="接口headers信息")
    url = Column(String, comment="接口地址")
    type = Column(Integer, comment="请求体类型：1:query，2:form-data，3:json")
    parameters = Column(String, comment="请求参数样例")
    description = Column(String, comment="接口描述")
    # server_path = Column(Integer, comment="接口服务器地址")  # 是否留用
    status = Column(Integer,default=1, comment="是否有效，默认有效")
...
class TestCaseInfo(Base):
    ...
    server_path = Column(Integer, comment="接口用例默认执行服务器服务器地址")
    belong_project = Column(Integer, comment="所属项目")
    related_case = Column(Integer, comment="关联测试用例id，优先执行关联用例")
    create_user = Column(Integer, comment="创建人id")
    update_user = Column(Integer, comment="更新人id")
    create_time = Column(DateTime, default=datetime.now, comment="创建时间")
    update_time = Column(DateTime, default=datetime.now, onupdate=datetime.now, comment="更新时间")
...
class TestCaseExecutionReport(Base):
    ...
    total_num = Column(Integer, default=0, comment="用例总数")
    success_num = Column(Integer, default=0, comment="执行成功数")
    fail_num = Column(Integer, default=0, comment="执行失败数")
    skip_num = Column(Integer, default=0, comment="跳过用例数")
    belong_project = Column(Integer, comment="所属项目id", default=None)
    create_user = Column(Integer, comment="创建人id")
    update_user = Column(Integer, comment="更新人id")
    create_time = Column(DateTime, default=datetime.now, comment="创建时间")
...
class TestCaseExecutionRecord(Base):
    ...
    actual_results = Column(String, comment="实际返回结果")
    create_user = Column(Integer, comment="创建人id")
    create_time = Column(DateTime, default=datetime.now, comment="创建时间")
    update_user = Column(Integer, comment="更新人id")
    update_time = Column(DateTime, default=datetime.now, onupdate=datetime.now, comment="更新时间")
